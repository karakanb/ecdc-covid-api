const calculateCaseDeathRatesPer100kForLastNDays = (rows, days) => {
    ...
    for (let index = 0; index < days; index++) {
        ...
        if (row && row.cases) {
            totalCaseCount += parseInt(row.cases, 10);
        }
        if (row && row.deaths) {
...
module.exports = { calculateCaseDeathRatesPer100kForLastNDays }